...
const CreateBodySchema = z.object({
  ...
  category: z.enum(['productivity', 'coding', 'creative', 'business', 'entertainment', 'education', 'other']).optional(),
...
const UpdateExposureBodySchema = z.object({
  ...
  public_profile: z.object({
    ...
    category: z.enum(['productivity', 'coding', 'creative', 'business', 'entertainment', 'education', 'other']),
    tos_required: z.boolean().optional(),
    tos_text: z.string().optional(),
    daily_quota: z.coerce.number().int().nonnegative().optional(),
    weekly_quota: z.coerce.number().int().nonnegative().optional(),
...
export default async function instanceRoutes(fastify: FastifyInstance) {
  ...
  fastify.patch('/instances/:id/exposure', { preHandler: [authenticateOrDevBypass] }, async (request, reply) => {
    const { id } = request.params as { id: string };
    const user = request.user;
    ...
    if (!validTransitions[from]?.includes(exposure)) {
      return reply.status(400).send({ error: `Invalid exposure transition: ${from} -> ${exposure}` });
    }

    const updateInput: Parameters<typeof instanceService.update>[1] = { exposure };
  ...
  fastify.post('/instances/:id/chat', async (request, reply) => {
    ...
    if (instance.exposure === 'public' && instance.tosRequired && !body.data.tos_acknowledged) {
  ...
  fastify.get('/me/shared-instances', { preHandler: [authenticateOrDevBypass] }, async (request, reply) => {
    const user = request.user;
  ...
  fastify.get('/discovery/search', async (request, reply) => {
    const { q, category, sort, page, per_page } = request.query as Record<string, string>;
    const searchQuery = q ?? '';
    const pageNum = Math.max(1, Number(page ?? 1));
    const perPageNum = Math.min(100, Math.max(1, Number(per_page ?? 20)));
    ...
    const ownerRows = hitIds.length > 0
      ? await db
          .select({ id: instances.id, namespace: users.namespace, displayName: users.displayName, avatarUrl: users.avatarUrl })
          .from(instances)
          .innerJoin(users, eq(instances.ownerId, users.id))
  ...
  fastify.get('/discovery/feed/:feed', async (request, reply) => {
    ...
    if (!['trending', 'new', 'featured'].includes(feed)) {
      return reply.status(400).send({ error: 'Invalid feed. Must be one of: trending, new, featured' });
    }
  ...
  fastify.get('/public/agents/:owner/:agentName', async (request, reply) => {
    const { owner, agentName } = request.params as { owner: string; agentName: string };
  ...
  // ── Public chat proxy ──────────────────────────────────────────────────────
  fastify.post('/public/agents/:owner/:agentName/chat', async (request, reply) => {
    const { owner, agentName } = request.params as { owner: string; agentName: string };
    ...
    const store = (fastify as any)._publicChatRateLimitStore ?? new Map<string, number[]>();
  ...
  fastify.get('/instances/:id/analytics', { preHandler: [authenticateOrDevBypass] }, async (request, reply) => {
    const { id } = request.params as { id: string };
    const user = request.user;
