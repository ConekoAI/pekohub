...
export function publicKeyFromDidKey(did: string): Uint8Array {
  ...
  if (!methodSpecificId.startsWith('z')) {
    throw new TunnelAuthError('Invalid did:key format: multibase base58btc prefix z missing');
  }
  ...
  if (pubKey.length !== 32) {
    throw new TunnelAuthError(`Invalid Ed25519 public key length: ${pubKey.length}`);
  }
